...
public Grafo(){
    ...
    void encontrarIsolados(){
        List<String> isolados = new ArrayList<>();

        for(String chave : grafo.keySet()){
            if(grafo.get(chave).size() < 1)
            isolados.add(chave);
        }

        if(isolados.isEmpty())
        System.out.println("\nNenhum vértice encontrado!\nTodos possuem, ao menos, uma conexão\n");

        else
        System.out.println("\nVértice(s) sem conexão: " + isolados);
    }

    void removerAresta(String inicio, String fim){
        if (!grafo.containsKey(inicio) || !grafo.containsKey(fim)) {
            System.out.println("\nPelo menos um de seus vértices(\"" + inicio + "\" e \"" + fim + "\") não existe.\nVerifique e tente novamente\n");
        }

        grafo.get(inicio).remove(fim);
        grafo.get(fim).remove(inicio);
        System.out.println("\nConexão removida!\n");
    }
}

public class intermediaria3{
